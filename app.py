...
def load_model():
    ...
    final_matrix = tfidf.fit_transform(data['overview'])
...
def recommend(movie_title, top_n=10, language=None):
    ...
    idx = matches.index[0]
    distances, indices = nn_model.kneighbors(final_matrix[idx], n_neighbors=min(len(data), 300))
    ...
    if 'weighted_score' not in recommendations.columns:
        recommendations['weighted_score'] = recommendations.get('vote_average', 0) / 10
